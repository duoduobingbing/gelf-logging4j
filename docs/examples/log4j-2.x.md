...
### Dynamic Field Typing
 ...
 In some cases, it's required to use a fixed type for fields transported using GELF. MDC is a dynamic value source 
 and since types can vary, so also data types in the GELF JSON vary. You can define `DynamicMdcFieldType` rules to declare types with Regex `Pattern`-based rules.
